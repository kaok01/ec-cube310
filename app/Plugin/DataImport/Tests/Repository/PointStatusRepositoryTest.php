...
/**
 * Class DataImportStatusRepositoryTest
 *
 * @package Eccube\Tests\Repository
 */
class DataImportStatusRepositoryTest extends EccubeTestCase
{
    public function testSelectOrderIdsWithUnfixedByCustomer()
    {
        ...
        // 検証
        $orderIds = $this->app['eccube.plugin.dataimport.repository.dataimportstatus']->selectOrderIdsWithUnfixedByCustomer(
            $customer->getId()
        );
    ...
    public function testIsFixStatusOnUnfixed()
    {
        ...
        // 検証
        $this->assertFalse(
            $this->app['eccube.plugin.dataimport.repository.dataimportstatus']->isFixedStatus(
                $order
            )
        );
    }
    ...
    {
        /** @var DataImportStatus $status */
        $status = $this->app['eccube.plugin.dataimport.repository.dataimportstatus']->findOneBy(array('order_id' => $delOrderId));
        $status->setDelFlg(1);
        $this->app['orm.em']->flush();
    }
    ...
    {
        /** @var DataImportStatus $status */
        $status = $this->app['eccube.plugin.dataimport.repository.dataimportstatus']->findOneBy(array('order_id' => $orderId));
        $status->setStatus(1);
        $this->app['orm.em']->flush();
    }
}
