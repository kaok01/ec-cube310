 ...
 */
class FrontDataImportControllerTest extends AbstractWebTestCase
{
     ...
     * カート画面のテストケース.
     */
    public function testDataImportCart()
    {
        $currentDataImport = 1000000;
    ...
    public function testDataImportShopping()
    {
        ...
        $DataImports = $this->app['eccube.plugin.dataimport.repository.dataimport']->findBy(array(
            'Customer' => $Customer,
            'Order' => $Order
        ));
        $provisionalDataImport = array_reduce(
            array_map(
                function ($DataImport) {
    ...
    public function testLongUseDataImportShopping()
    {
        ...
        for ($i = 0; $i < $purchaseNum; $i++) {
            ...
            // 完了画面
            $crawler = $this->scenarioComplete($client, $this->app->path('shopping_confirm'));
            $provisionalAddDataImport += $addDataImport;
        }
    ...
    public function testLongUseDataImportShoppingWithSubtraction()
    {
        ...
        for ($i = 0; $i < $purchaseNum; $i++) {
            ...
            // 完了画面
            $crawler = $this->scenarioComplete($client, $this->app->path('shopping_confirm'));
            $provisionalAddDataImport += $addDataImport - ($discount * ($DataImportInfo->getPlgBasicDataImportRate() / 100));
        ...
        $deleted = 0;
        $fixAddDataImport = 0;
        $deleteUseDataImport = 0;
        $useDataImportOrderIds = array();
        ...
        foreach ($useDataImportOrderIds as $order_id) {
            ...
            $changeUseDataImport = 200;
            $changeAddDataImport = 1098;
            $this->saveOrder($UseDataImportOrder, $changeUseDataImport, $changeAddDataImport);
            $currentDataImport -= ($changeUseDataImport - $useDataImport);
            $currentDataImport += ($changeAddDataImport - ($addDataImport - ($discount * ($DataImportInfo->getPlgBasicDataImportRate() / 100))));
        }
     ...
     * お届け先変更をした場合のテストケース.
     *
     * @link https://github.com/EC-CUBE/dataimport-plugin/issues/114
     */
    ...
    protected function scenarioUseDataImport($client, $useDataImport)
    {
        $crawler = $client->request(
            'POST',
            $this->app->path('dataimport_use'),
            array('front_dataimport_use' =>
                  array(
                      'plg_use_dataimport' => $useDataImport,
                      '_token' => 'dummy'
                  )
            )
        );
    ...
    /**
     * AdminOrder::fixDataImport() を実行する.
     *
     * protected なので リフレクションを使用する.
     */
    protected function fixDataImport($Order, $Customer)
    {
        $AdminOrder = new AdminOrder();
        $Reflect = new \ReflectionClass($AdminOrder);
        $Method = $Reflect->getMethod('fixDataImport');
     ...
     */
    protected function saveOrder($Order, $useDataImport = 0, $addDataImport = 0)
    {
        $form = array(
            'use_dataimport' => new FormTypeMock($useDataImport),
            'add_dataimport' => new FormTypeMock($addDataImport)
        );
        $AdminOrder = new AdminOrder();
        $event = new EventArgs(
            array(
